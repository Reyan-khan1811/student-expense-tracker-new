...
export default function Dashboard() {
  ...
  const getDaysMessage = () => {
    if (totalSpent === 0) return "♾️ Forever — start spending!";
    if (daysUntilBroke === Infinity) return "♾️ Forever";
      ...
      >
        ...
        <div className="flex items-end gap-4">
          <div className="flex-1">
            ...
            >
              {totalSpent === 0 ? '♾️' : remaining <= 0 ? '0' : daysUntilBroke === Infinity ? '♾️' : daysUntilBroke}
            </motion.p>
          ...
          <div className="text-right">
             ...
             <div className={`px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-wider ${daysStyle.bg}/10 ${daysStyle.color}`}>
               {getDaysMessage()}
             </div>
          </div>
        </div>
